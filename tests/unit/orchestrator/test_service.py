...
class FakeCrawler:
    ...
    async def fetch(self, req) -> Document:
...
def make_result(url: str, rank: int) -> Result:
    return Result(
        url=url, title=f"title {rank}", snippet=f"snippet {rank}", engine="brave", rank=rank
    )
...
def make_document(url: str, *, markdown: str, content_hash: str, status: str = "ok") -> Document:
    return Document(
        ...
        enveloped_markdown=f'<untrusted_source url="{url}" sha256="{content_hash}">{markdown}</untrusted_source>'
        if status == "ok"
        else "",
...
def test_orchestrator_emits_happy_path_events_in_order() -> None:
    ...
    assert events[-1].payload["documents"] == [
        {"url": "https://example.com/a", "content_hash": "doc-1"}
    ]
...
def test_orchestrator_skips_page_ready_for_failed_crawls() -> None:
    search = FakeSearchClient(
        SearchResponse(
            results=[
                make_result("https://example.com/a", 0),
                make_result("https://example.com/b", 1),
            ],
            provider="brave",
            query_sent="python",
            latency_ms=20,
    ...
    crawler = FakeCrawler(
        {
            "https://example.com/a": make_document(
                "https://example.com/a",
                markdown="Python content with enough detail.",
                content_hash="doc-1",
            ),
            "https://example.com/b": make_document(
                "https://example.com/b", markdown="blocked", content_hash="doc-2", status="http_4xx"
            ),
        }
    )
    llm = FakeLLMClient(
    ...
    orchestrator = Orchestrator(
        search_client=search, crawl_client=crawler, llm_client=llm, config=OrchestratorConfig()
    )
...
def test_orchestrator_returns_ungrounded_answer_when_all_crawls_fail() -> None:
    ...
    crawler = FakeCrawler(
        {
            "https://example.com/a": make_document(
                "https://example.com/a", markdown="missing", content_hash="doc-1", status="timeout"
            )
        }
    )
    llm = FakeLLMClient(
